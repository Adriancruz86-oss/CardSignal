...
export function correlateCatalystEvent(event:CatalystEvent,history:ScanSnapshot[],now=Date.now()):CatalystOutcomeRow[]{
 ...
 for(const cardId of cardIds){
  ...
  const before=rows.filter(x=>ms(x.scannedAt)<=eventAt).sort((a,b)=>ms(b.scannedAt)-ms(a.scannedAt))[0];
  const afterNear=rows.filter(x=>ms(x.scannedAt)>eventAt&&ms(x.scannedAt)<=eventAt+12*3600000).sort((a,b)=>ms(a.scannedAt)-ms(b.scannedAt))[0];
  ...
  const points:CatalystOutcomePoint[]=OUTCOME_HORIZONS.map(h=>{
   ...
   const candidates=rows.filter(x=>ms(x.scannedAt)>=target-h.toleranceHours*3600000&&ms(x.scannedAt)<=target+h.toleranceHours*3600000&&x.currentMedian!=null&&x.currentMedian>0).sort((a,b)=>Math.abs(ms(a.scannedAt)-target)-Math.abs(ms(b.scannedAt)-target));
