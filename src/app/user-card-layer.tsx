...
type UserCard = {
  ...
  liveValuation?: {provider?:string;identityLabel?:string;confidence?:string;median?:number;compCount?:number;savedAt?:string};
};
...
function readCards():UserCard[]{
  try{
    ...
    return raw.map(card=>({...card,id:Number(card.id||Date.now()),score:Number(card.score||0),marketValue:Number(card.marketValue||0),mode:card.mode==="owned"?"owned":"watching",tone:card.tone==="buy"||card.tone==="sell"?card.tone:"hold",purchasePrice:card.purchasePrice===undefined||card.purchasePrice===""?undefined:Number(card.purchasePrice)}));
  }catch{return[]}
}
...
export default function UserCardLayer(){
  const[cards,setCards]=useState<UserCard[]>([]),[open,setOpen]=useState(false),[tab,setTab]=useState<"owned"|"watching">("owned"),[search,setSearch]=useState(""),[sort,setSort]=useState<SortMode>("score"),[filter,setFilter]=useState<FilterMode>("ALL");
  ...
  useEffect(()=>{
    ...
    document.addEventListener("click",onClickCapture,true);return()=>document.removeEventListener("click",onClickCapture,true);
  },[]);
  ...
  useEffect(()=>{
    const watchPanel=document.querySelector<HTMLElement>(".triple-grid .radar-panel:nth-child(3)");if(!watchPanel)return;
    watchPanel.querySelectorAll(".cs-added-row").forEach(row=>row.remove());
    [...watching].reverse().forEach(card=>{
      const row=document.createElement("div");row.className="signal-row cs-added-row cs-restored-row";row.dataset.userCardId=String(card.id);const sc=getCardSignalScore(card);
      ...
      const firstRow=watchPanel.querySelector(".signal-row");if(firstRow)watchPanel.insertBefore(row,firstRow);else watchPanel.appendChild(row);
    });
  },[cards,watching]);
  ...
  const removeCard=(id:number)=>{const card=cards.find(item=>item.id===id);if(!card)return;if(!window.confirm(`Remove ${card.player} from CardSignal?`))return;const next=cards.filter(item=>item.id!==id);saveCards(next);setCards(next);window.dispatchEvent(new Event("cardsignal:user-cards-changed"));try{const detailState=JSON.parse(localStorage.getItem("cardsignal-card-detail-state")||"{}");delete detailState[`${card.player}|${card.meta}`];localStorage.setItem("cardsignal-card-detail-state",JSON.stringify(detailState))}catch{}};

  const activeCards=useMemo(()=>{
    const base=(tab==="owned"?owned:watching).filter(card=>{const q=search.trim().toLowerCase(),text=[card.player,card.meta,card.year,card.setName,card.cardNumber,card.variant].filter(Boolean).join(" ").toLowerCase(),searchOk=!q||text.includes(q),signal=card.marketScan?.acceptedCount&&card.marketScan.acceptedCount>=3?card.marketScan.pulse:"NOT ENOUGH DATA";return searchOk&&(filter==="ALL"||signal===filter)});
    return [...base].sort((a,b)=>sort==="score"?getCardSignalScore(b).score-getCardSignalScore(a).score:sort==="value"?Number(b.marketValue||0)-Number(a.marketValue||0):sort==="move"?Math.abs(moveNumber(b))-Math.abs(moveNumber(a)):sort==="recent"?scanTime(b)-scanTime(a):a.player.localeCompare(b.player));
  },[tab,owned,watching,search,sort,filter]);

  return <>{open&&<div className="cs-portfolio-backdrop" onMouseDown={event=>event.target===event.currentTarget&&setOpen(false)}><section className="cs-portfolio-modal" role="dialog" aria-modal="true" aria-label="My CardSignal cards">
    ...
    <div className="cs-portfolio-stats"><div><small>OWNED MARKET VALUE</small><strong>${marketTotal.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong><span>{owned.length} owned card{owned.length===1?"":"s"}</span></div><div><small>COST BASIS</small><strong>${costBasis.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong><span>From entered purchase prices</span></div><div><small>UNREALIZED GAIN / LOSS</small><strong className={gainLoss>=0?"positive":"negative"}>{gainLoss>=0?"+":"-"}${Math.abs(gainLoss).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong><span>{costBasis>0?`${gainLoss>=0?"+":""}${((gainLoss/costBasis)*100).toFixed(1)}%`:"Add purchase prices to calculate"}</span></div><div><small>WATCHING</small><strong>{watching.length}</strong><span>Personal watchlist cards</span></div></div>
    ...
    <div className="cs-portfolio-controls"><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search player, set, card #, variant…" aria-label="Search cards"/><select value={filter} onChange={e=>setFilter(e.target.value as FilterMode)} aria-label="Filter by signal"><option value="ALL">All signals</option><option>BUY MORE</option><option>SELL RISK</option><option>WATCH CLOSELY</option><option>HOLD</option><option>NOT ENOUGH DATA</option></select><select value={sort} onChange={e=>setSort(e.target.value as SortMode)} aria-label="Sort cards"><option value="score">Score: highest</option><option value="value">Value: highest</option><option value="move">Biggest 7D move</option><option value="recent">Most recently scanned</option><option value="name">Player A–Z</option></select><span>{activeCards.length} shown</span></div>
    <div className="cs-portfolio-list">{activeCards.length===0?<div className="cs-portfolio-empty"><span className="cs-portfolio-empty-mark">▦</span><b>No cards match these filters</b><p>Clear the search or signal filter to see more cards.</p></div>:activeCards.map(card=>{const sc=getCardSignalScore(card),s=card.marketScan;return <article className="cs-portfolio-card signal-row" data-user-card-id={card.id} key={card.id}><div className={`cs-portfolio-art ${card.tone}`}>{card.image?<img src={card.image} alt={card.player}/>:<span>{initials(card.player)}</span>}</div><div className="cs-portfolio-copy"><strong>{card.player}</strong><span>{card.meta}</span><small>{s?`${s.acceptedCount} accepted matches · ${s.confidence} confidence`:card.mode==="owned"&&card.purchasePrice?`Paid $${card.purchasePrice.toFixed(2)}`:card.mode==="owned"?"Not scanned yet":"Watching for an entry"}</small></div><div className="cs-portfolio-market"><small>{s?"CURRENT MEDIAN":"MARKET"}</small><strong>${Number(card.marketValue||0).toFixed(2)}</strong><span className={(s?.change7d??0)<0?"negative":"positive"}>{s?.change7d==null?"—":`${s.change7d>=0?"+":""}${s.change7d.toFixed(1)}% 7D`}</span></div><div className={`cs-portfolio-score ${sc.tone}`}><small>CARDSIGNAL SCORE</small><strong>{sc.score||"—"}</strong><span>{sc.label}</span></div><div className="cs-portfolio-actions"><button onClick={e=>{e.stopPropagation();updateMode(card.id,card.mode==="owned"?"watching":"owned")}}>{card.mode==="owned"?"MOVE TO WATCHLIST":"MARK AS OWNED"}</button><button className="danger" onClick={e=>{e.stopPropagation();removeCard(card.id)}}>REMOVE</button></div></article>})}</div>
  </section></div>}<style jsx global>{`
