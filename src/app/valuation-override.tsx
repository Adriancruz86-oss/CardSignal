  ...
  useEffect(() => {
    const enhance = () => {
      ...
      if (mainButton?.disabled && target && accepted > 0 && median != null) {
        ...
        override.textContent = accepted === 1 ? "APPLY 1 COMP ANYWAY" : "APPLY LOW-CONFIDENCE VALUE";
        override.onclick = () => {
          ...
          const index = cards.findIndex((card) => card.player === player && (!meta || card.meta === meta));
          const fallback = index >= 0 ? index : cards.findIndex((card) => card.player === player);
          if (fallback < 0) {
