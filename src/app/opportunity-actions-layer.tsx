...
function findCard(row:HTMLElement){const list=cards(),id=Number(row.dataset.userCardId||0);if(id){const byId=list.find(c=>c.id===id);if(byId)return byId}const player=row.querySelector<HTMLElement>(".cs-op-title strong")?.textContent?.trim()||"",meta=row.querySelector<HTMLElement>(".cs-op-title small")?.textContent?.trim()||"";if(!player)return null;const exact=list.filter(c=>norm(c.player)===norm(player)&&(!meta||norm(c.meta)===norm(meta)));if(exact.length===1)return exact[0];const same=list.filter(c=>norm(c.player)===norm(player));return same.length===1?same[0]:null}
...
export default function OpportunityActionsLayer(){
 ...
 useEffect(()=>{
  ...
  const click=async(e:MouseEvent)=>{
   ...
   if(act==="watch"){const next:Card[]=cards().map(c=>c.id===card.id?{...c,mode:c.mode==="watching"?"owned" as const:"watching" as const}:c);const updated=next.find(c=>c.id===card.id)!;saveCards(next);log(updated,updated.mode==="watching"?"WATCHLIST":"OWNED",row);b.textContent=updated.mode==="watching"?"✓ WATCHING":"✓ OWNED";return}
