...
export default function CatalystIntegrationLayer(){
 ...
 const actionItems=useMemo(()=>{
  const cards=readCards();const byPlayer=new Map<string,Card[]>();for(const card of cards){const key=card.player.toLowerCase();byPlayer.set(key,[...(byPlayer.get(key)||[]),card])}const rows:{card:Card;article:Article;fetchedAt:string}[]=[];
  for(const [key,result] of Object.entries(cache)){
   const matching=byPlayer.get(key)||[];const article=strongArticles(result)[0];if(!article)continue;
   for(const card of matching)rows.push({card,article,fetchedAt:result.fetchedAt});
  }
  return rows.sort((a,b)=>b.article.impact-a.article.impact).slice(0,8);
 },[cache]);
 ...
 return <>
  ...
  {actionHost&&actionItems.length>0&&createPortal(<section className="cs-ac-catalysts"><div className="cs-ac-cat-title"><span>STRONG CATALYSTS</span><b>News worth reviewing by exact saved card</b></div>{actionItems.map(({card,article,fetchedAt})=><button key={`${card.id}-${article.url}`} onClick={()=>openCard(card.id)} className={toneClass(article.tone)}><div><strong>{card.player}</strong><span>{card.meta||"Saved card"} · {article.category} · impact {article.impact} · {ageLabel(fetchedAt)}</span></div><p>{article.title}</p></button>)}</section>,actionHost)}
  <style jsx global>{`
   ...
   .cs-ac-catalysts{margin:0 0 12px;padding:10px;border:1px solid rgba(238,200,110,.15);border-radius:10px;background:rgba(41,32,12,.12)}.cs-ac-cat-title{display:flex;justify-content:space-between;align-items:end;margin-bottom:7px}.cs-ac-cat-title span{color:#efc86e;font-size:8px;font-weight:900;letter-spacing:.12em}.cs-ac-cat-title b{color:#93adba;font-size:8px;font-weight:700}.cs-ac-catalysts>button{width:100%;display:grid;grid-template-columns:200px 1fr;gap:10px;text-align:left;padding:9px;border:0;border-top:1px solid rgba(95,185,219,.08);background:transparent;color:#dcecf2;cursor:pointer}.cs-ac-catalysts>button:first-of-type{border-top:0}.cs-ac-catalysts>button:hover{background:rgba(14,39,53,.55)}.cs-ac-catalysts strong,.cs-ac-catalysts span,.cs-ac-catalysts p{display:block}.cs-ac-catalysts strong{font-size:10px}.cs-ac-catalysts span{margin-top:3px;color:#708f9f;font-size:7px}.cs-ac-catalysts p{margin:0;color:#9fb6c1;font-size:9px;line-height:1.35}.cs-ac-catalysts>button.negative strong{color:#ff8997}.cs-ac-catalysts>button.positive strong{color:#6befaa}.cs-ac-catalysts>button.watch strong{color:#efc86e}@media(max-width:680px){.cs-ac-catalysts>button{grid-template-columns:1fr}.cs-cat-int-row{grid-template-columns:60px 1fr}}
  `}</style>
 </>;
}
