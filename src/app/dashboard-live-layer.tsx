...
type Card = { id:number; player:string; meta?:string; mode?:"owned"|"watching"; marketValue?:number; score?:number; move?:string; tone?:"buy"|"hold"|"sell"; image?:string; demo?:boolean; catalogConfirmed?:boolean; catalogSource?:string; marketScan?:MarketScan; liveValuation?:{compCount?:number;median?:number;confidence?:string;savedAt?:string} };
...
export default function DashboardLiveLayer(){
 useEffect(()=>{
  const render=()=>{
   ...
   const watching=cards.filter(c=>c.mode==="watching");
   ...
   if(stats[0]){stats[0].querySelector(".stat-copy>span")!.textContent="PORTFOLIO VALUE";stats[0].querySelector(".stat-copy>strong")!.textContent=money(portfolioValue);stats[0].querySelector(".stat-copy>small")!.textContent=`${enough.length} of ${owned.length} owned cards have market evidence`;}
   ...
   if(top){const s=top.marketScan;setText(".top-signal .panel-heading h2",top.player);setText(".top-signal .panel-heading p",top.meta||"Saved portfolio card");setText(".top-signal .live-dot",s?"● PORTFOLIO SIGNAL":"● NOT SCANNED");setText(".top-signal .player-mark span",initials(top.player));setText(".top-signal .massive-score",String(top.score||0));setHTML(".top-signal .recommendation",`<span>${s?.pulse==="SELL RISK"?"▼":s?.pulse==="BUY MORE"?"▲":"◎"}</span> ${safe(s?.pulse||"NEEDS DATA")} <small>${safe(s?.confidence||"LOW")} CONFIDENCE</small>`);setText(".top-signal .signal-primary > p",s?(s.acceptedCount>=3?`${s.acceptedCount} exact/strong comps accepted. ${s.change7d==null?"Not enough dated history yet for a trustworthy 7-day comparison.":`Recent-vs-prior median movement is ${pct(s.change7d)}.`}`:"Fewer than 3 accepted comps. CardSignal is withholding a market signal."):"Run Portfolio Pulse to collect exact-card market evidence for this card.");setText(".top-signal .price-line strong",s?.currentMedian!=null?`$${s.currentMedian.toFixed(2)}`:"—");setText(".top-signal .price-line small",s?.change7d!=null?`${pct(s.change7d)} / 7D`:"trend not established");setText(".top-signal .updated",s?`Last scanned ${new Date(s.scannedAt).toLocaleString()}`:"Not scanned yet");}
   else {setText(".top-signal .panel-heading h2","Add your first card");setText(".top-signal .panel-heading p","Your strongest real portfolio signal will appear here.");setText(".top-signal .massive-score","—");setText(".top-signal .signal-primary > p","Add cards, then run Portfolio Pulse. CardSignal will only surface signals backed by accepted market comps.");setText(".top-signal .price-line strong","—");}

   const breakdown=document.querySelectorAll<HTMLElement>(".breakdown .metric");
   const topScan=top?.marketScan;
   const metrics:[string,number,string][]=[
    ["7D price move",topScan?.change7d==null?0:Math.min(100,Math.abs(topScan.change7d)*4),topScan?.change7d==null?"Not enough dated sales":pct(topScan.change7d)],
    ["Sales velocity",topScan?.velocity||0,topScan?`${topScan.recentSales} recent accepted sales`:"Not scanned"],
    ["Exact comp evidence",topScan?Math.min(100,topScan.acceptedCount*8):0,topScan?`${topScan.acceptedCount} accepted · ${topScan.rejectedCount} rejected`:"Not scanned"],
   ...
   const alerts=document.querySelector<HTMLElement>(".alerts-panel");if(alerts){alerts.querySelectorAll(".alert").forEach(n=>n.remove());const alertCards=[...scanned].sort((a,b)=>new Date(b.marketScan!.scannedAt).getTime()-new Date(a.marketScan!.scannedAt).getTime()).filter(c=>c.marketScan!.pulse!=="HOLD").slice(0,3);alertCards.forEach(c=>{const s=c.marketScan!;const cls=s.pulse==="SELL RISK"?"sell":s.pulse==="BUY MORE"?"buy":"info";const icon=s.pulse==="SELL RISK"?"▼":s.pulse==="BUY MORE"?"▲":"◎";const a=document.createElement("div");a.className=`alert ${cls}`;a.innerHTML=`<i>${icon}</i><div><b>${safe(s.pulse)}</b><p>${safe(c.player)} · ${s.acceptedCount} accepted comps${s.change7d==null?"":` · ${safe(pct(s.change7d))} 7D`}</p><small>${new Date(s.scannedAt).toLocaleString()}</small></div>`;alerts.appendChild(a)});if(!alertCards.length){const a=document.createElement("div");a.className="alert info";a.innerHTML=`<i>◎</i><div><b>No material portfolio alerts</b><p>${scanned.length?"Scanned cards are currently HOLD or need more evidence.":"Run Portfolio Pulse to generate evidence-backed alerts."}</p><small>${latestScan?new Date(latestScan).toLocaleString():"No scan yet"}</small></div>`;alerts.appendChild(a)}}
   ...
   setHTML(".snapshot-panel .market-note",`<span>◎</span><p><b>${enough.length} of ${owned.length} owned cards have usable comp evidence</b><br>${avgChange==null?"Not enough dated sales for an aggregate 7D move.":`Average measured 7D move: ${safe(pct(avgChange))}.`} ${needs.length} cards need more exact comps.</p>`);
 ...
 return <style jsx global>{`.cs-live-home-row .mini-card{overflow:hidden}.cs-live-home-row .mini-card img{position:absolute;inset:2px;width:calc(100% - 4px);height:calc(100% - 4px);object-fit:cover;border-radius:5px}.cs-trend-empty{height:190px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:30px;border:1px dashed rgba(80,190,225,.16);border-radius:10px;background:rgba(5,20,32,.34)}.cs-trend-empty b{color:#c5dce7;font-size:13px}.cs-trend-empty span{max-width:520px;margin-top:8px;color:#69899a;font-size:10px;line-height:1.55}`}</style>;
}
