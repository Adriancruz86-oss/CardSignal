...
export default function CardLeagueEditorLayer(){
 ...
 const card=useMemo(()=>{void version;const cards=readCards(),p=norm(player),m=norm(meta);return cards.find(c=>norm(c.player)===p&&m&&norm(c.meta)===m)||cards.find(c=>norm(c.player)===p&&m&&norm([c.year,c.setName,c.cardNumber&&`#${c.cardNumber}`,c.variant].filter(Boolean).join(" · "))===m)||cards.find(c=>norm(c.player)===p)||null},[player,meta,version]);
