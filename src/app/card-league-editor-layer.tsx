...
export default function CardLeagueEditorLayer(){
 const[host,setHost]=useState<HTMLElement|null>(null),[cardId,setCardId]=useState<number|null>(null),[player,setPlayer]=useState(""),[meta,setMeta]=useState(""),[version,setVersion]=useState(0);
 useEffect(()=>{const attach=()=>{const modal=document.querySelector<HTMLElement>(".cs-detail-modal");if(!modal){setHost(null);setCardId(null);setPlayer("");setMeta("");return}const head=modal.querySelector<HTMLElement>(".cs-detail-head");setCardId(Number(modal.dataset.userCardId||0)||null);setPlayer(head?.querySelector<HTMLElement>("h2")?.textContent?.trim()||"");setMeta(head?.querySelector<HTMLElement>("p")?.textContent?.trim()||"");let h=modal.querySelector<HTMLElement>("#cs-league-editor-host");if(!h){h=document.createElement("div");h.id="cs-league-editor-host";const perf=modal.querySelector("#cs-performance-host");if(perf)perf.insertAdjacentElement("beforebegin",h);else modal.appendChild(h)}setHost(h)};attach();const obs=new MutationObserver(attach);obs.observe(document.body,{childList:true,subtree:true});const r=()=>setVersion(v=>v+1);window.addEventListener("cardsignal:user-cards-changed",r);return()=>{obs.disconnect();window.removeEventListener("cardsignal:user-cards-changed",r)}},[]);
 const card=useMemo(()=>{void version;const cards=readCards();if(cardId!=null){const byId=cards.find(c=>c.id===cardId);if(byId)return byId}const p=norm(player),m=norm(meta),samePlayer=cards.filter(c=>norm(c.player)===p);if(!samePlayer.length)return null;if(m){const exact=samePlayer.filter(c=>norm(c.meta)===m);if(exact.length===1)return exact[0];const identity=samePlayer.filter(c=>norm([c.year,c.setName,c.cardNumber&&`#${c.cardNumber}`,c.variant].filter(Boolean).join(" · "))===m);if(identity.length===1)return identity[0]}return samePlayer.length===1?samePlayer[0]:null},[cardId,player,meta,version]);
 ...
 if(!card)return createPortal(<div className="cs-league-editor cs-league-unresolved"><div><span>LEAGUE</span><b>Exact card unresolved</b></div><small>CardSignal will not apply a league override unless the open card resolves to one saved card ID.</small>{styles}</div>,host);
