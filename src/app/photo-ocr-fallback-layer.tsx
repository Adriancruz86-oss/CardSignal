...
function findCardNumber(back: string, backTop: string) {
  ...
  const special = all.match(/\b([A-Z]{1,4}\d{1,4})\b/)?.[1] || "";
  if (special && !/^(RC|MLB)\d/i.test(special)) return { value: special, confidence: "high" as const };

  for (const value of lines(backTop).slice(0, 10)) {
    const direct = value.match(/^#?\s*([A-Z0-9-]{1,8})$/i)?.[1] || "";
    if (!direct || /^(19|20)\d{2}$/.test(direct)) continue;
    if (/^\d{1,3}$/.test(direct) || /[A-Za-z]/.test(direct)) return { value: direct, confidence: "high" as const };
  }
  for (const value of lines(back).slice(0, 18)) {
    const direct = value.match(/^#?\s*([A-Z0-9-]{1,8})$/i)?.[1] || "";
...
function findYear(combined: string, backBottom: string) {
  ...
  return fallback.length ? { value: String(Math.min(...fallback)), confidence: "low" as const } : { value: "", confidence: "low" as const };
}
...
async function cropAndEnhance(file: File, x: number, y: number, w: number, h: number) {
  ...
  try {
    const img = await new Promise<HTMLImageElement>((resolve, reject) => {
      const node = new Image(); node.onload = () => resolve(node); node.onerror = () => reject(new Error("Could not preprocess card image")); node.src = url;
    });
    ...
    const ctx = canvas.getContext("2d"); if (!ctx) throw new Error("Could not preprocess card image");
...
export default function PhotoOcrFallbackLayer() {
  ...
  useEffect(() => {
    const evaluate = () => setTarget(document.querySelector<HTMLElement>(".cs-photo-actions"));
    evaluate();
  ...
  const runOcr = async () => {
    ...
    const front = files[0]?.files?.[0]; const back = files[1]?.files?.[0];
    if (!front) return;

    setVisible(true); setWorking(true); setStatus("Reading factual clues from the card…"); setClues(null);
    ...
    try {
      ...
      worker = await createWorker("eng", 1, { logger: (m) => { if (m.status === "recognizing text" && typeof m.progress === "number") setStatus(`Reading card text… ${Math.round(m.progress * 100)}%`); } });
      ...
      const next = extractClues(frontResult.data.text || "", backResult?.data.text || "", frontBottomResult.data.text || "", backTopResult?.data.text || "", backBottomResult?.data.text || "");
      setClues(next);
      ...
      setStatus(query ? "Factual clues extracted — searching CardSight's catalog for canonical matches…" : "OCR finished. Nothing low-confidence was written into the card form.");
    } catch (error) {
      setStatus(error instanceof Error ? `OCR failed: ${error.message}` : "OCR failed");
    } finally {
      await worker?.terminate(); setWorking(false);
    }
  };

  useEffect(() => {
    const handler = () => { void runOcr(); };
    window.addEventListener("cardsignal:request-ocr", handler);
    return () => window.removeEventListener("cardsignal:request-ocr", handler);
  });

  if (!target || !visible) return null;
  return createPortal(
    <div className="cs-ocr-fallback">
      <div className="cs-ocr-fallback-head"><span>PHOTO TEXT VERIFICATION</span><b>OCR supplies clues. The catalog supplies identity.</b></div>
      <button type="button" onClick={runOcr} disabled={working}>{working ? "READING TARGETED REGIONS…" : "READ / VERIFY FRONT + BACK"}</button>
      <small>CardSignal does not trust visual recognition or OCR by itself. It uses these clues to find a canonical catalog card for you to confirm.</small>
