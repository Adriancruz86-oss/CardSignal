...
type Candidate = {
  year: string | null; brand: string | null; line: string | null; format: string | null;
  category: "Sports" | "Pokémon"; canonicalKey: string; confidence: "high" | "medium" | "low";
  evidenceCount: number; pricedEvidenceCount: number; marketMedian: number | null;
  lastSaleDate: string | null; sampleTitle: string; marketplace: string;
};
type ResponseData = { ok: boolean; provider?: string; fetchedAt?: string; scannedListings?: number; candidates?: Candidate[]; identityNote?: string; error?: string };
...
export default function LiveSealedDiscovery() {
  ...
  async function search(event: FormEvent) {
    ...
    try {
      ...
      setResult(await response.json() as ResponseData);
    } catch (error) {
  ...
  async function confirm(candidate: Candidate) {
    ...
    setSaveMessage((old) => ({ ...old, [key]: "" }));
    try {
      await persistConfirmedSealed(candidate, result?.provider);
      setConfirmed((old) => ({ ...old, [key]: true }));
      setSaveMessage((old) => ({ ...old, [key]: "Saved to your sealed catalog and recorded a market snapshot." }));
      window.dispatchEvent(new Event("cardsignal-sealed-snapshot"));
    } catch (error) {
      setSaveMessage((old) => ({ ...old, [key]: error instanceof Error ? error.message : "Could not save sealed product." }));
    } finally {
  ...
  return (
    <section className="live-sealed-discovery"><div className="live-sealed-inner">
      <div className="live-sealed-heading"><div><span>LIVE SEALED DISCOVERY</span><h2>Find the product first. Confirm the exact SKU second.</h2><p>Search live sold-market evidence, group listings into product identities, then confirm the exact wax product. Confirming now saves the SKU to your account and records a market snapshot for trend history.</p></div><b>{result?.provider ? `LIVE · ${result.provider}` : "PROVIDER READY"}</b></div>
      <form onSubmit={search} className="live-sealed-search"><select value={category} onChange={(e) => setCategory(e.target.value as "Sports" | "Pokémon")}><option>Sports</option><option>Pokémon</option></select><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Try: 2026 Topps Chrome hobby box" /><button disabled={loading || query.trim().length < 3}>{loading ? "Searching…" : "Search live market"}</button></form>
      {result && !result.ok && <div className="live-sealed-error">{result.error || "Live discovery failed."}</div>}
      {result?.ok && <>
        <div className="live-sealed-meta"><span>{result.scannedListings ?? 0} marketplace records scanned</span><span>{result.candidates?.length ?? 0} sealed identities found</span><span>{result.fetchedAt ? `Fetched ${new Date(result.fetchedAt).toLocaleString()}` : ""}</span></div>
        {result.identityNote && <p className="identity-warning">{result.identityNote}</p>}
        <div className="identity-grid">{(result.candidates ?? []).map((candidate) => {
          const isConfirmed = Boolean(confirmed[candidate.canonicalKey]);
          const isSaving = Boolean(saving[candidate.canonicalKey]);
          return <article className="identity-card" key={candidate.canonicalKey}>
            <div className="identity-head"><div><span>{candidate.category} · {candidate.format ?? "Format unknown"}</span><h3>{[candidate.year, candidate.brand, candidate.line].filter(Boolean).join(" ")}</h3></div><b className={`identity-confidence ${candidate.confidence}`}>{candidate.confidence} ID</b></div>
            <div className="identity-evidence"><div><span>MARKET MEDIAN</span><b>{money(candidate.marketMedian)}</b></div><div><span>EVIDENCE</span><b>{candidate.evidenceCount}</b></div><div><span>PRICED SALES</span><b>{candidate.pricedEvidenceCount}</b></div><div><span>LAST SALE</span><b>{candidate.lastSaleDate ? candidate.lastSaleDate.slice(0,10) : "—"}</b></div></div>
            <div className="canonical-key"><span>CANONICAL SKU KEY</span><code>{candidate.canonicalKey}</code></div>
            <details><summary>See source listing title</summary><p>{candidate.sampleTitle}</p></details>
            <button className={isConfirmed ? "confirmed" : ""} disabled={isSaving || isConfirmed} onClick={() => confirm(candidate)}>{isSaving ? "Saving…" : isConfirmed ? "✓ Saved + snapshot recorded" : "Confirm + track exact SKU"}</button>
