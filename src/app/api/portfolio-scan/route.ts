...
function exactMatch(c:Canonical,s:Sale){const t=normalize(s.title);if(LOT_WORDS.some(x=>phraseIn(t,x)))return false;const pt=normalize(c.player).split(" ").filter(x=>x.length>1);if(pt.length&&!pt.every(x=>t.includes(x)))return false;if(c.year){const years=[...t.matchAll(/\b((?:19|20)\d{2})\b/g)].map(x=>x[1]);if(!years.includes(c.year.slice(0,4)))return false}if(c.setName){const required=setTokens(c.setName);if(required.length&&!required.every(x=>t.includes(x)))return false}if(c.cardNumber){const n=extractNumber(t);if(!n||normalize(n)!==normalize(c.cardNumber))return false}if(c.variant&&!phraseIn(t,c.variant))return false;if(!c.variant&&VARIANTS.some(v=>phraseIn(s.title,v)))return false;return true}
...
export async function GET(request:NextRequest){
  ...
  const q=[c.player,c.year,c.setName,c.cardNumber?`#${c.cardNumber}`:"",c.variant].filter(Boolean).join(" ");
