...
function exactMatch(c:Canonical,s:Sale){const t=normalize(s.title);const pt=normalize(c.player).split(" ").filter(x=>x.length>1);if(pt.length&&!pt.every(x=>t.includes(x)))return false;if(c.year){const y=t.match(/\b((?:19|20)\d{2})\b/)?.[1]??"";if(y&&y!==c.year.slice(0,4))return false}if(c.setName&&!phraseIn(t,c.setName))return false;if(c.cardNumber){const n=extractNumber(t);if(n&&normalize(n)!==normalize(c.cardNumber))return false}if(c.variant&&!phraseIn(t,c.variant))return false;if(!c.variant&&VARIANTS.some(v=>phraseIn(s.title,v)))return false;return true}
...
export async function GET(request:NextRequest){
  ...
  const accepted=raw.filter(s=>exactMatch(c,s)&&s.price!=null&&s.price>0);
