...
export default function SafePortfolioLayer(){
 ...
 const openCard=(id:number)=>{setOpen(false);window.setTimeout(()=>window.dispatchEvent(new CustomEvent("cardsignal:open-card-detail",{detail:{cardId:id}})),40)};
 if(!open)return null;
 return <div className="cs-safe-portfolio-backdrop" onMouseDown={e=>e.target===e.currentTarget&&setOpen(false)}>
  <section className="cs-safe-portfolio" role="dialog" aria-modal="true" aria-label="Portfolio">
   ...
   <div className="list">{shown.length===0?<div className="empty">No cards match this view.</div>:shown.map(c=><article key={c.id} className="cs-portfolio-card" data-user-card-id={c.id}>
    ...
    <div className="actions"><button className="open-card" onClick={()=>openCard(c.id)}>OPEN CARD</button><button onClick={()=>move(c.id,c.mode==="owned"?"watching":"owned")}>{c.mode==="owned"?"MOVE TO WATCHLIST":"MARK AS OWNED"}</button><button className="danger" onClick={()=>remove(c.id)}>REMOVE</button></div>
   </article>)}</div>
  </section>
