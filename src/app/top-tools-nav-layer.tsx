...
const LAB: Tool[] = [
  ...
  {
    label: "Scan Ready",
    selector: ".cs-benchmark-readiness-launch",
    icon: "◷",
  },
...
export default function TopToolsNavLayer() {
  ...
  useEffect(() => {
    ...
    const navHandler = (event: MouseEvent) => {
      ...
      } else if (label === "Buy Radar") {
        event.preventDefault();
        scrollTo(".triple-grid .radar-panel:nth-child(1)");
      } else if (label === "Sell Radar") {
        event.preventDefault();
        scrollTo(".triple-grid .radar-panel:nth-child(2)");
      } else if (label === "Watchlist") {
        event.preventDefault();
        scrollTo(".triple-grid .radar-panel:nth-child(3)");
      }
    };
    document.addEventListener("click", navHandler, true);
    const viewAllHandler = (event: MouseEvent) => {
      const button = (event.target as HTMLElement).closest<HTMLButtonElement>(
        ".radar-panel .section-title button",
      );
      if (!button) return;
      const panel = button.closest<HTMLElement>(".radar-panel");
      const panels = [
        ...document.querySelectorAll<HTMLElement>(".triple-grid .radar-panel"),
      ];
      ...
      ].find((b) => b.textContent?.trim() === labels[idx]);
      nav?.click();
    };
    document.addEventListener("click", viewAllHandler, true);
    return () => {
      document.removeEventListener("click", navHandler, true);
      document.removeEventListener("click", viewAllHandler, true);
    };
  }, []);
  ...
  return createPortal(
    <>
      <div className="cs-top-tools-inner">
        <span className="cs-top-tools-label">CARDSIGNAL</span>
        {CORE.map((t) => (
        ...
        <div className="cs-lab-wrap">
          <button
            className={labOpen ? "lab active" : "lab"}
      ...
      <style jsx global>{`
        .cs-decision-brief-launch,
        .cs-pulse-launch,
        .cs-edge-stack-launch,
        .cs-analogs-launch,
        .cs-playbook-launch,
        .cs-scorecard-launch,
        .cs-organizer-launch,
        .cs-sell-prep-launch,
        .cs-signal-lab-launch,
        .cs-sources-launch,
        .cs-live-launch,
        .cs-catalyst-launch,
        .cs-cat-history-launch,
        .cs-outcome-launch,
        .cs-validation-cohort-launch,
        .cs-discovery-launch,
        .cs-market-scout-launch,
        .cs-performance-watch-launch,
        .cs-pop-alert-launch,
        .cs-segment-launch,
        .cs-pokemon-launch,
        .cs-benchmark-launch,
        .cs-benchmark-health-launch,
        .cs-benchmark-readiness-launch,
        .cs-opportunity-launch,
        .cs-journal-launch,
        .cs-ac-launch {
          display: none !important;
        }
        #cardsignal-top-tools {
          position: relative;
          z-index: 140;
        ...
        .cs-lab-wrap > button {
          ...
          align-items: center;
          gap: 6px;
          border: 1px solid rgba(74, 188, 228, 0.16);
          border-radius: 7px;
        ...
        .cs-top-tools-inner > button:first-of-type {
          border-color: rgba(89, 235, 159, 0.3);
          color: #a2f5c4;
        }
        .cs-top-tools-inner button:hover {
          border-color: rgba(80, 220, 255, 0.34);
          color: #d9f7ff;
          background: rgba(10, 38, 56, 0.9);
        }
        .cs-top-tools-inner button i {
          font-style: normal;
          color: #57dcff;
        }
        .cs-top-tools-inner > button:first-of-type i {
          color: #68efa9;
        }
        .cs-lab-wrap {
          position: relative;
        ...
        .cs-lab-menu {
          ...
          gap: 6px;
          border: 1px solid rgba(80, 199, 239, 0.2);
          border-radius: 11px;
          background: rgba(4, 18, 29, 0.99);
          box-shadow: 0 24px 70px rgba(0, 0, 0, 0.7);
        }
        .cs-lab-menu > div {
          grid-column: 1/-1;
          padding: 4px 4px 8px;
          border-bottom: 1px solid rgba(80, 190, 225, 0.08);
        }
        .cs-lab-menu > div span,
        .cs-lab-menu > div small {
          display: block;
        }
        .cs-lab-menu > div span {
          color: #68dcff;
        ...
        .cs-lab-menu > button {
          ...
          align-items: center;
          gap: 7px;
          border: 1px solid rgba(75, 187, 225, 0.1);
          border-radius: 7px;
          background: rgba(7, 27, 41, 0.75);
          color: #89aebb;
          font-size: 8px;
          font-weight: 800;
          text-align: left;
        ...
        .cs-nav-flash {
          animation: csNavFlash 0.9s ease;
        }
        @keyframes csNavFlash {
          0%,
          100% {
            box-shadow: 0 0 0 rgba(79, 218, 255, 0);
          }
          35% {
            box-shadow:
              0 0 0 2px rgba(79, 218, 255, 0.24),
              0 0 30px rgba(79, 218, 255, 0.12);
          }
        }
